...
@router.post("/start")
async def start_session(request_body: ChatStartRequest, current_user=Depends(get_current_user)):
...
async def continue_session(chat_id: str, request_body: ChatContinueRequest, current_user=Depends(get_current_user)):
    service = get_guided_learning_service()
    return await service.continue_session(current_user.id, chat_id, request_body)


@router.post("/{chat_id}/continue/stream")
async def continue_session_stream(chat_id: str, request_body: ChatContinueRequest, current_user=Depends(get_current_user)):
    service = get_guided_learning_service()
...
async def complete_session(chat_id: str, request_body: ChatCompleteRequest, current_user=Depends(get_current_user)):
    service = get_guided_learning_service()
    return await service.complete_session(current_user.id, chat_id, request_body)


@router.get("/{chat_id}/history")
async def get_history(chat_id: str, current_user=Depends(get_current_user)):
    service = get_guided_learning_service()
