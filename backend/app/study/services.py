...
def get_pending_self_assessments(db: Session, user: User):
    """
    ...
    # Query principal: inscrições sem SELF_ASSESSMENT
    pending_subscriptions = (
        db.query(UserContest)
        .options(joinedload(UserContest.role).joinedload(ContestRole.contest))
        .filter(
            UserContest.user_id == user.id,
            ~UserContest.id.in_(
                db.query(UserTopicProgress.user_contest_id)
                .filter(UserTopicProgress.id.in_(assessed_user_contest_ids))
            )
        )
        .all()
    )
...
def update_user_proficiency_by_subject(db: Session, user: User, user_contest_id: int, submission: ProficiencySubmission):
    ...
    if existing_self_assessment:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="Você já enviou sua autoavaliação para esta inscrição."
        )
        ...
        if review_session:
            for topic in review_session.topics:
                ...
                if review_progress:
                    ...
                    review_progress.sessions_studied += 1
